...
def convert_image_annotations(original_image_metadata,
    ...
    for i in tqdm(range(num_images), mininterval=0.5):
        ...
        if image_size is not None:
            ...
            img['height'], img['width'], _ = cv2.imread(filename).shape
            # img['width'], img['height'] = imagesize.get(filename) # for some rare images, imagesize fails and outputs the wrong(transposed) shape.
