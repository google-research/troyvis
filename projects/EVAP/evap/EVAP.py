...
from .models.evap_model import EVAP_Model
...
class EVAP(nn.Module):
    """
    Implement EVA-Perceiver
    """
    ...
    def __init__(self, cfg):
        ...
        self.glee = EVAP_Model(cfg, matcher, self.device, self.video_info, cfg.MODEL.CONTRAS_MEAN)
    ...
    def forward(self, batched_inputs):
        ...
        if self.training:
            ...
            # In prepare_targets function, prompt_list will be updated
        ...
        if self.reuse_kernel:
            ...
            for idx in range(video_len):
                ...
                if idx % self.reuse_kernel_clip_length == 0: # run decoder only on key frames
                    (clip_output,_),dist,loss = self.glee(clip_images, {}, task,  batch_name_list = batch_name_list, is_train= False, mask_kernels=None)
                    ...
                    (clip_output,_),dist,loss = self.glee(clip_images, {}, task,  batch_name_list = batch_name_list, is_train= False, mask_kernels=mask_kernels)
...
def rle_encode_gpu(mask):
    ...
    assert len(mask.shape) == 2, "Input mask must be 2D"
    
    h, w = mask.shape
    # mask = mask.to(torch.uint8)
    
    # Flatten the mask (equivalent to F-order)
    flat_mask = mask.t().flatten()
    
    # Find where the mask changes
    diff = flat_mask[1:] != flat_mask[:-1]
    change_indices = torch.cat([torch.tensor([0], device=mask.device), diff.nonzero().flatten() + 1])
    
    # Compute run lengths
    run_lengths = change_indices[1:] - change_indices[:-1]
    
    # Add the length of the last run
    run_lengths = torch.cat([run_lengths, torch.tensor([flat_mask.size(0) - change_indices[-1]], device=mask.device)])
    
    # Convert to CPU and numpy for further processing
    run_lengths = run_lengths.cpu().numpy()
    
    # Encode using the modified COCO RLE encoding
    counts = []
    for i, x in enumerate(run_lengths):
        ...
        while more:
            c = x & 0x1f
            x >>= 5
            more = (c & 0x10) != 0 and x != -1 or (c & 0x10) == 0 and x != 0
            if more:
                c |= 0x20
            counts.append(c + 48)
...
def rle_encode_gpu_batch(masks):
    assert masks.dtype == torch.bool, "Input masks must be boolean tensors"
    assert len(masks.shape) == 3, "Input masks must be 3D (N, H, W)"
    
    N, H, W = masks.shape
    
    # Flatten the masks (equivalent to F-order)
    flat_masks = masks.transpose(1, 2).reshape(N, -1)
    
    results = []

    diff = flat_masks[:, 1:] ^ flat_masks[:, :-1] # XOR could be much faster

    for i in range(N):
        # Find where the mask changes
        
        change_indices = torch.cat([torch.tensor([0], device=masks.device), diff[i].nonzero().flatten() + 1])
        # Compute run lengths
        run_lengths = change_indices[1:] - change_indices[:-1]
        
        # Add the length of the last run
        run_lengths = torch.cat([run_lengths, torch.tensor([flat_masks.size(1) - change_indices[-1]], device=masks.device)])
        
        # Convert to CPU and numpy for further processing
        run_lengths = run_lengths.cpu().numpy()
        counts = []
        for j, x in enumerate(run_lengths):
            if j > 2:
                x -= run_lengths[j-2]
            more = True
            while more:
