...
from .models.evap_model_dpl_onnx import EVAP_Model_DPL_ONNX
...
__all__ = ["EVAP_DPL_ONNX"]



@META_ARCH_REGISTRY.register()
class EVAP_DPL_ONNX(nn.Module):
    """
    Implement EVAP_DPL_ONNX (for deployment only)
    """
    ...
    def __init__(self, cfg):
        ...
        self.glee = EVAP_Model_DPL_ONNX(cfg, self.device, self.video_info, cfg.MODEL.CONTRAS_MEAN).eval() # to eval mode
    ...
    def MinVIS_inference(self, batched_inputs, task):
        ...
        if self.reuse_kernel:
            ...
            for idx in range(video_len):
                ...
                # pred_masks.append(clip_output['pred_masks'].squeeze(0).cpu()) #.to(self.merge_device)
        else:
    ...
    def minvis_inference_video(self, mask_cls, mask_pred, mask_box_result, img_size, output_height, output_width, first_resize_size, task, ori_logits, batched_inputs):
        if task != 'tao_video':
            if len(mask_cls) > 0:
                ...
                # mask_pred = mask_pred[topk_indices.cpu()] # (topk, L, H/4, W/4)
                mask_box_result = mask_box_result[topk_indices]
                if self.is_lsj:
