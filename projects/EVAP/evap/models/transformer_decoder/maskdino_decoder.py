...
class MaskDINODecoder(nn.Module):
    ...
    def forward(self, x, mask_features, extra, task, masks, targets=None, mask_kernels=None):
        """
        ...
        """
        if (not self.training) and (mask_kernels is not None):
            # during inference and reuse previous mask kernels
            outputs_mask = torch.einsum("bqc,bchw->bqhw", mask_kernels, mask_features)
        ...
        if self.two_stage:
            ...
            conf_score, outputs_class, outputs_mask, _, _ = self.forward_prediction_heads(tgt_undetach.transpose(0, 1), mask_features, task, extra, mask_dict = None)
        ...
        if self.initial_pred:
            conf_score, outputs_class, outputs_mask, pred_federat, _ = self.forward_prediction_heads(tgt.transpose(0, 1), mask_features, task, extra, mask_dict, self.training)
        ...
        for i, output in enumerate(hs):
            conf_score, outputs_class, outputs_mask,pred_federat, outputs_kernels = self.forward_prediction_heads(output.transpose(0, 1), mask_features, task, extra, mask_dict, self.training or (i == len(hs)-1))
            predictions_score.append(conf_score)
            predictions_class.append(outputs_class)
            predictions_mask.append(outputs_mask)
            predictions_federate.append(pred_federat)
            predictions_kernel.append(outputs_kernels)
        ...
        out = {
            ...
            'pred_kernels': predictions_kernel[-1],
            'visual_P': visual_P,
    ...
    def forward_prediction_heads(self, output, mask_features, task, extra,mask_dict, pred_mask=True, visual_P=False):
        ...
        outputs_mask = None
        mask_embed = None # kernels for mask prediction (b, q, c)
        if pred_mask:
            mask_embed = self.mask_embed(decoder_output)
            outputs_mask = torch.einsum("bqc,bchw->bqhw", mask_embed, mask_features)


        return conf_score, outputs_class, outputs_mask, None, mask_embed
...
class VL_Align(nn.Module):
    def __init__(self, cfg):
        ...
        self.dot_product_projection_text = nn.Linear(cfg.MODEL.DIM_PROJ, cfg.MODEL.MaskDINO.HIDDEN_DIM, bias=True) # 768 -> 256
        self.log_scale = nn.Parameter(torch.Tensor([cfg.MODEL.DYHEAD.LOG_SCALE]), requires_grad=True)
        self.bias_lang = nn.Parameter(torch.zeros(cfg.MODEL.DIM_PROJ), requires_grad=True) # (768，)
        self.bias0 = nn.Parameter(torch.Tensor([bias_value]), requires_grad=True) # size (1,)
    
    def forward(self, x, embedding):
        """
        x: visual features (bs, num_query, 256)
        embedding: language features (bs, L, 256)
        """
        # norm
...
@TRANSFORMER_DECODER_REGISTRY.register()
class MaskDINODecoderNew(nn.Module):
    @configurable
    def __init__(
            self,
            in_channels,
    ...
    def forward(self, x, mask_features, extra, task, masks, targets=None, mask_kernels=None):
        ...
        if self.two_stage:
            ...
            if task in ['grounding','rvos']:
                ...
                # class_embed = output_memory @ self.category_embed  # [bz,num_q,projectdim]
                # enc_outputs_class_unselected = torch.einsum("bqc,nc->bqn", class_embed, extra['class_embeddings'])  #[bz,n,80]
                enc_outputs_class_unselected = self.match_head(output_memory, extra['class_embeddings'].unsqueeze(0).repeat(len(output_memory), 1, 1))  #[bz,n,80]
    ...
    def forward_prediction_heads(self, output, mask_features, task, extra,mask_dict, pred_mask=True, visual_P=False):
        ...
        if task in ['grounding', 'rvos']:
            ...
            # outputs_class = torch.einsum("bqc,nc->bqn", class_embed, extra['class_embeddings'])  #[bz,n,80]
            outputs_class = self.match_head(decoder_output, extra['class_embeddings'].unsqueeze(0).repeat(len(decoder_output), 1, 1))  #[bz,n,80]
