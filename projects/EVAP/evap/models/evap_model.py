...
class EVAP_Model(nn.Module):
    ...
    def __init__(self, cfg, matcher, device, video_info, contras_mean):
        ...
        if cfg.MODEL.TEXT.ARCH == 'clip_frozen':
            ...
            self.tokenizer.add_special_tokens({'cls_token': self.tokenizer.eos_token})
            self.text_encoder = CLIPTextModel.from_pretrained(cfg.MODEL.TEXT.DIR)
            self.text_encoder_teacher = CLIPTextModel.from_pretrained(cfg.MODEL.TEXT.DIR)
            self.lang_encoder = None
            ...
            # fix visual tower
            for p in self.text_encoder.visual.parameters():
                p.requires_grad = False
            self.tokenizer = get_tokenizer(model_name)
            # self.tokenizer.add_special_tokens({'cls_token': self.tokenizer.eos_token})
            self.lang_encoder = None
            if cfg.MODEL.TEXT.ARCH == 'EVA02_L_CLIP_frozen':
                for p in self.text_encoder.parameters():
                    p.requires_grad = False
            if cfg.MODEL.TEXT.ARCH == 'EVA02_L_CLIP_teacher':
                self.text_encoder_teacher = create_model(model_name, pretrained, force_custom_clip=True)
                for p in self.text_encoder_teacher.parameters():
                    p.requires_grad = False
            self.lang_projection = nn.Parameter(torch.rand(cfg.MODEL.LANGUAGE_BACKBONE.LANG_DIM, cfg.MODEL.DIM_PROJ))
        if cfg.SOLVER.FREEZE_TEXT_ENCODER:
            for p in self.text_encoder.parameters():
                p.requires_grad = False
    ...
    def forward(self, images, prompts, task, targets=None, batch_name_list=None, is_train = True, visual_prompt_type='scribble', mask_kernels=None):
        extra =  {}
        # dist_loss = None
        early_semantic = None 
        ...
        if self.text_encode_type == 'clip_frozen':
            ...
            if task not in ['grounding','rvos']:
                ...
                if is_train and self.text_encode_type == 'EVA02_L_CLIP_teacher':
                    lang_feat_final_teacher = self.text_encoder_teacher(text)
                    dist_loss = F.mse_loss(lang_feat_final, lang_feat_final_teacher)
                else:
                    dist_loss =  (lang_feat_final*0).sum()
                lang_feat_final = lang_feat_final @ self.lang_projection
                extra['class_embeddings'] = lang_feat_final # (L, C)
                if self.early_fusion: # class early_fusion
                    gather_all_classtoken = lang_feat_final.unsqueeze(0).repeat(len(images),1,1) #[bs,L,C]
                    gather_all_classtoken_mask = torch.ones_like(gather_all_classtoken[:,:,0])>0  #[bs,L]
                    early_semantic = {"hidden":gather_all_classtoken.float(),"masks":gather_all_classtoken_mask} 
        elif self.text_encode_type == "clip_teacher": # True during inference
            if task not in ['grounding','rvos']:
                assert batch_name_list
                calsses_name_list = batch_name_list
