...
class MSDeformAttnTransformerEncoder(nn.Module):
    def __init__(self, vl_fusion_layer, encoder_layer, num_layers, early_fusion, effi_early_fusion, effi_lvl):
        super().__init__()
        ...
        if early_fusion:
            ...
            self.vl_layers = [None] * num_layers
        self.effi_early_fusion = effi_early_fusion
        self.effi_lvl = effi_lvl
    ...
    def forward(self, src, spatial_shapes, level_start_index, valid_ratios, pos=None, padding_mask=None, early_fusion=None):
        ...
        for _, (layer,vl_layer) in enumerate(zip(self.layers, self.vl_layers)):
            if early_fusion:
                ...
                if self.effi_early_fusion:
                    L = 0 # number of tokens not used for X-Att
                    for l in range(self.effi_lvl):
                        h, w = spatial_shapes[l]
                        L += h * w
                    output_eff = {"visual": output["visual"][:, L:, :], "lang": output["lang"]}
                    # torch.cuda.synchronize()
                    # t0 = time.time()
                    output_eff = vl_layer(output_eff)
                    # torch.cuda.synchronize()
                    # t1 = time.time()
                    # print("early fusion:", t1-t0)
                    output["visual"] = torch.cat([output["visual"][:, :L, :], output_eff["visual"]], dim=1)
...
class MaskDINOEncoder(nn.Module):
    ...
    def forward_features(self, features, masks, early_fusion=None):
        ...
        # torch.cuda.synchronize()
        # t3 = time.time()
        # print("proj:%.4f, trans enc:%.4f, fpn:%.4f"%(t1-t0, t2-t1, t3-t2))
        return self.mask_features(out[-1]), out[0], multi_scale_features, zero_loss, srcs
